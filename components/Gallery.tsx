...
const Gallery: React.FC<GalleryProps> = ({ 
  ...
  // Helper to generate a consistent color/initial for avatar
  ...
  return (
    <div className="flex flex-col min-h-full">
      ...
      <div className="pt-24 pb-8 px-6 md:px-12 bg-gradient-to-b from-white/40 to-transparent">
        <div className="max-w-4xl mx-auto flex flex-col md:flex-row items-center md:items-start gap-6 md:gap-10">
          
          {/* Avatar */}
          <div className="relative group shrink-0">
             <div className="w-24 h-24 md:w-32 md:h-32 rounded-full border-4 border-white shadow-xl overflow-hidden bg-gray-200 flex items-center justify-center">
                {userProfile ? (
          ...
          <div className="flex-1 text-center md:text-left space-y-3">
             <div className="flex flex-col md:flex-row items-center gap-3">
                <h2 className="text-2xl md:text-3xl font-mono font-bold text-gray-900 tracking-tight">
                   {userProfile ? userProfile.username : t(lang, 'profile.guest')}
                </h2>
                {userProfile ? (
                  <button onClick={onEditProfile} className="px-3 py-1 text-xs border border-gray-300 rounded-full hover:bg-black hover:text-white transition-colors flex items-center gap-1">
                     <Edit2 size={12} /> {t(lang, 'profile.edit')}
                  </button>
                ) : (
                  <span className="px-2 py-0.5 bg-gray-200 text-gray-600 text-[10px] rounded uppercase tracking-wider font-bold">Guest Mode</span>
                )}
             </div>

             <p className="text-sm text-gray-500 font-mono max-w-md mx-auto md:mx-0">
             ...
             <div className="flex items-center justify-center md:justify-start gap-6 text-sm font-mono pt-2">
                <div>
                   <span className="font-bold text-lg block text-black">{photos.length}</span>
                   <span className="text-gray-500 text-xs uppercase">{t(lang, 'profile.shots')}</span>
                </div>
                {/* Could add 'Following' or 'Likes' here in future */}
             </div>
          </div>
          ...
          <div className="shrink-0">
             {userProfile ? (
               ...
               <button 
                 onClick={onLogin} 
                 className="bg-black text-white px-6 py-2 rounded-full font-bold shadow-lg hover:scale-105 transition-transform flex items-center gap-2"
               >
                 <UserCircle2 size={18} />
      ...
      <div className="flex-1 px-2 md:px-4 pb-40 max-w-6xl mx-auto w-full">
         <div className="border-t border-gray-300/50 mb-8 mx-6"></div>
         
         {photos.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-20 text-gray-400 opacity-60 min-h-[30vh]">
